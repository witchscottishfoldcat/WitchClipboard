...
- 传输全程端到端加密：会话密钥由配对 token 与设备 id 派生，文字、图片、文件分块均以 AES-256-GCM 加密，配对 token 只在扫码加载页面时上线一次
- 文件按 256 KB 分片加密，显示进度，可取消；上传以稳定传输 ID 和 `.part` 文件恢复，下载按块序号续传
- 关闭连接后二维码立即失效；数据不经过第三方服务器
- Key、Token、密码默认拒绝发送；文字上限 100 KB、图片上限 20 MB、单文件上限 2 GB

这是一项局域网便利功能：加密能抵御配对完成后的被动嗅探，但配对瞬间二维码与配对页仍在明文 http 上传输，仍建议只在可信 Wi‑Fi 中开启。
